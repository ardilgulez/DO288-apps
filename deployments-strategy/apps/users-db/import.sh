...
while [ "$HOOK_RETRIES" != 0 ]; do
  ...
  if mysqlshow --user=$MYSQL_USER --password=$MYSQL_PASSWORD $MYSQL_DATABASE &>/dev/null
  then
    echo 'Database is up'
    break
  else
...
# Run the SQL script
if mysql --user=$MYSQL_USER --password=$MYSQL_PASSWORD $MYSQL_DATABASE < /post-deploy/users.sql
